...
/*
 * HK algorithm with current-arc (iterator) optimization
 * Given a current matching M, HK performs phases: 
 *   (1) BFS (layering): Build a level graph from all free (unmatched)
 *       vertices in L as sources, layering by shortest alternating
 *       paths (edges alternate between non-matching and matching edges),
...
private:
    ...
    // it[u] = current-arc pointer: index of the next edge in adj[u] that
    // dfs() should try. Reset to 0 at the start of every phase. Because a
    // reference to it[u] drives the DFS loop, an edge that fails to advance
    // is skipped permanently for the rest of the phase and never retried,
    // which makes the per-phase work a tight O(E).
    vector<int> it;
     ...
     * O(E)
     *
     * The current-arc pointer it[u] is what enforces "each edge at most
     * once": the loop advances it[u] as edges fail, so subsequent entries
     * into dfs(u) resume from where the previous one stopped.
     */
    bool dfs(int u) {
...
public:
    ...
    int maximumMatching() 
    {
         ...
         */
        while (bfs()) {
            // Reset current-arc pointers at the start of each phase so the
            // per-phase DFS work is a tight O(E).
            it.assign(n, 0);
